...
# --- AutoConfirm: simple polling watcher (no webhook required) ---
def _spawn_payment_watch(payment_id: str, telegram_id: int, interval_sec: int = 10, timeout_min: int = 25):
    """Polls Mercado Pago until approved, then activates DB record and notifies the user."""
    def _runner():
        try:
            max_loops = max(1, int((timeout_min*60)/interval_sec))
            for _ in range(max_loops):
                st = None
                try:
                    st = payment_service.check_payment_status(str(payment_id))
                except Exception as e:
                    logger.error(f"[WATCH] status error: {e}")
                if st and st.get("success") and (st.get("paid") or st.get("status") == "approved"):
                    # Activate in DB
                    try:
                        with db_service.get_session() as session:
                            db_user = session.query(User).filter_by(telegram_id=str(telegram_id)).first()
                            if not db_user:
                                logger.error(f"[WATCH] user not found: {telegram_id}")
                                return
                            expires = _activate_user_subscription(session, db_user, str(payment_id))
                    except Exception as e:
                        logger.error(f"[WATCH] DB activation error: {e}")
                        return
                    # Telegram notify
                    try:
                        from config import Config as _Cfg
                        ...
                        logger.error(f"[WATCH] Telegram notify error: {e}")
                    return
                time.sleep(interval_sec)
            logger.warning(f"[WATCH] timeout payment_id={payment_id}")
        except Exception as e:
            logger.error(f"[WATCH] unexpected: {e}")
    try:
        threading.Thread(target=_runner, daemon=True).start()
        logger.info(f"[WATCH] started payment_id={payment_id} user={telegram_id}")
    except Exception as e:
        logger.error(f"[WATCH] spawn error: {e}")


# --- AutoConfirm: monkey-patch payment_service.create_subscription_payment to auto-spawn watcher ---
def __wrap_create_subscription_payment_autowatch():
    try:
        svc = payment_service
        if not hasattr(svc, "create_subscription_payment"):
            logger.warning("[WATCH] payment_service has no create_subscription_payment")
            return
        if getattr(svc, "__wrapped_autowatch__", False):
            return  # already wrapped

        _orig = svc.create_subscription_payment
        ...
        def _wrapped(*args, **kwargs):
            ...
            try:
                ...
                # Bubble up after logging; original behavior preserved
                logger.error(f"[WATCH] create_subscription_payment raised: {e}")
                raise
            try:
                # Extract telegram id from kwargs or first arg
                tg = kwargs.get("user_telegram_id")
                if tg is None and len(args) > 0:
                    tg = args[0]
                pid = None
                if isinstance(res, dict):
                    pid = res.get("payment_id") or ((res.get("raw") or {}).get("id") if res.get("raw") else None)
                if tg and pid:
                    _spawn_payment_watch(str(pid), int(str(tg)))
            except Exception as e:
                logger.error(f"[WATCH] post-create spawn failed: {e}")
            return res

        svc.create_subscription_payment = _wrapped
        svc.__wrapped_autowatch__ = True
        logger.info("[WATCH] create_subscription_payment wrapped for auto-confirm watcher")
    except Exception as e:
        logger.error(f"[WATCH] wrap error: {e}")

# Call the wrapper at import time (safe; it only swaps the method)
try:
    __wrap_create_subscription_payment_autowatch()
except Exception as _we:
    logger.error("[WATCH] auto-wrap failed: " + str(_we))
