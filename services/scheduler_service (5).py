...
class SchedulerService:
    ...
    def start(self):
        ...
        # Schedule timezone-aware daily reset check every minute
        # This will check for midnight in São Paulo timezone
        schedule.every().minute.do(self._check_midnight_reset)
    ...
    def _check_reminder_times(self):
        """Check if it's time for any user's scheduled reminders or reports - FIXED TIMEZONE ISSUE"""
        try:
            ...
            # Use Brazil timezone (America/Sao_Paulo) - CRITICAL FOR CORRECT TIMING
            brazil_tz = pytz.timezone('America/Sao_Paulo')
            current_datetime = datetime.now(brazil_tz)
            current_time_str = current_datetime.strftime("%H:%M")
            current_date = current_datetime.date()  # São Paulo local date
            current_time = current_datetime.time()
            
            logger.info(f"⏰ Checking reminder times at {current_time_str} (São Paulo local time)")
            logger.info(f"📅 São Paulo local date: {current_date}")
            ...
            with db_service.get_session() as session:
                ...
                for user, settings in users_settings:
                    ...
                    logger.info(f"🔍 User {user.id}: configured={morning_time_str}, current={current_time_str}, last_run={settings.last_morning_run}")
                    
                    # FIXED LOGIC: Check timing window and daily execution guard
                    # 1. Must be in the correct time window (not just >=)
                    time_window_start = daily_time
                    time_window_end = (datetime.combine(date.today(), daily_time) + timedelta(minutes=5)).time()
                    
                    # 2. Must not have run today (São Paulo local date)
                    already_ran_today = (settings.last_morning_run == current_date)
                    
                    in_time_window = time_window_start <= current_time <= time_window_end
                    
                    logger.info(f"🎯 User {user.id}: window={time_window_start}-{time_window_end}, in_window={in_time_window}, already_ran={already_ran_today}")
                    
                    # Execute only if in time window AND not executed today
                    if in_time_window and not already_ran_today:
                        logger.info(f"✅ EXECUTING: Daily reminders for user {user.id} at {current_time_str} (São Paulo)")
                        try:
                            # Process daily reminders
                            self._process_daily_reminders_sync(user.id)
                            
                            # Update last run date to São Paulo local date
                            settings.last_morning_run = current_date
                            session.commit()
                            
                            logger.info(f"✅ COMPLETED: Daily reminders for user {user.id}, marked as run on {current_date}")
                        except Exception as e:
                            logger.error(f"❌ Error processing daily reminders for user {user.id}: {str(e)}")
                            import traceback
                            logger.error(f"Full traceback: {traceback.format_exc()}")
                    elif already_ran_today:
                        logger.info(f"⏭️  SKIPPED: User {user.id} already ran today ({current_date})")
                    elif not in_time_window:
                        logger.debug(f"⏱️  WAITING: User {user.id} outside time window")
                    
                    # Daily report processing disabled during test phase
                    logger.debug(f"📊 Daily report processing disabled for user {user.id}")
    ...
    def _check_midnight_reset(self):
        """Check if it's midnight in São Paulo timezone and execute daily reset"""
        try:
            ...
            # Check if it's exactly midnight (00:00) in São Paulo
            if current_time.hour == 0 and current_time.minute == 0:
                logger.info(f"🕛 MIDNIGHT DETECTED: Starting daily reset at {current_datetime.strftime('%H:%M')} (São Paulo time)")
                self._execute_daily_reset()
                
        except Exception as e:
            logger.error(f"❌ Error checking midnight reset: {e}")
            import traceback
            logger.error(traceback.format_exc())
    
    def _execute_daily_reset(self):
        """Execute the daily reset at midnight São Paulo time to clear 'already sent today' flags"""
        try:
            # Use Brazil timezone (America/Sao_Paulo)
            brazil_tz = pytz.timezone('America/Sao_Paulo')
            current_datetime = datetime.now(brazil_tz)
            ...
            logger.info(f"🔄 MIDNIGHT RESET: Starting daily reset at {current_time_str} (São Paulo time)")
            ...
            with db_service.get_session() as session:
                ...
                logger.info(f"✅ MIDNIGHT RESET: Reset reminder flags for {updated_count} active clients")
                logger.info(f"🔄 MIDNIGHT RESET: Daily reset completed successfully at {current_time_str}")
                
        except Exception as e:
            logger.error(f"❌ Error in midnight daily reset: {e}")
            import traceback
            logger.error(traceback.format_exc())
